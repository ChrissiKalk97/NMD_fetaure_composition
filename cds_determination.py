...
def determine_cds(transcript_gtftk_object, transcript_ids_wo_cds, reference_file, reference_sequences_file):
    ...
    intersection = transcripts_several_orfs_bed.intersect(pc_reference_bed, wo = True).saveas()
    unique_names = set()
    # Iterate over features in the BedTool object and extract names
    for feature in intersection:
        if feature.name:
            unique_names.add(re.split( r':', feature.name)[1])
    print('nr tids directly after intersection', len(unique_names))
   ...
   # print(transcripts_with_CDS.tail())
