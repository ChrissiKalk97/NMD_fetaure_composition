...
def handle_cds_transcripts(transcript_gtftk_object, transcript_ids, NMD_features_df):
    ...
    for transcript_id in transcript_ids:
        ...
        if len(cds) > 0:
            stop_pos_genome, cds_length = find_termination_codon(
                transcript_info, cds)
            ...
            NMD_features_df.loc[transcript_id,
                                'last_exon_length'] = stop_pos_transcript['length'] - last_ejc
            NMD_features_df.loc[transcript_id,
                                'end_ORF'] = stop_pos_transcript['stop_position']
            ...
            NMD_features_df.loc[transcript_id,
                                't_length'] = stop_pos_transcript['length']
            NMD_features_df.loc[transcript_id,
                                'start_ORF'] = stop_pos_transcript['stop_position'] - cds_length
            NMD_features_df.loc[transcript_id,
                                'exon_with_stop_length'] = exon_containing_stop_length
            NMD_features_df.loc[transcript_id,
                                'distance_stop_next_EJC'] = dist_stop_next_EJC
