...
def determine_cds(transcript_gtftk_object, transcript_ids_wo_cds,\
    ...
    SeqIO.write(ORFs, f'Output/{output_name}_ORFS_protein_coding_genes.fasta', 'fasta') 
    ...
    transcripts_of_interest = transcripts_with_CDS['name'].str.split(r':|\|').str[1]
    sequences = [seq for seq in sequences if seq.id in transcripts_of_interest.values]
    
    SeqIO.write(sequences, f'Output/{output_name}_transcripts_of_interest.fasta', 'fasta')
