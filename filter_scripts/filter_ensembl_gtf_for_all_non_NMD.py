...
def main():
    ...
    transcript_ids = ensembl_gtf.select_by_key('feature', 'CDS')\
        .select_by_key('transcript_biotype', 'protein_coding')\
            .extract_data("transcript_id", as_list=True)
    transcript_string = ''
    for transcript in transcript_ids[:-1]:
        transcript_string += transcript+","
    transcript_string += transcript_ids[-1]
    no_nmd = ensembl_gtf.select_by_key('feature', 'transcript,exon,CDS').select_by_key('transcript_id', transcript_string)#.select_by_key('transcript_biotype', 'nonsense_mediated_decay', invert_match= True)
