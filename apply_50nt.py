...
def get_length_last_exon(transcript_ids_list, gtf_file):
    ...
    for transcript_id in transcript_ids_list:
        ...
        transcript_exons = [transcript_exons[x:x+5]
                            for x in range(0, len(transcript_exons), 5)]
        ...
        last_exon_length_by_transcript[transcript_id] = int(
            last_exon[1]) - int(last_exon[0]) + 1
...
def calculate_50nt_rule(transcripts_with_CDS: pd.DataFrame, sequences) -> pd.DataFrame:
    ...
    transcripts_with_CDS['t_length'] = transcripts_with_CDS['tid'].map(
        t_length_dict)
    ...
    transcripts_with_CDS['50_nt_rule'] = transcripts_with_CDS['distance_stop_EJC'].ge(
        50).astype(int)
